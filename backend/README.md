...
### 1. 문서 완료 현황 추적 (Document Progress Tracking) ✅

사용자별 정책 문서 준비 현황을 데이터베이스에 저장하고 추적하는 기능입니다.

#### 개요

- **목적**: PlanScreen의 체크박스 상태를 데이터베이스에 영구 저장
- **효과**: 페이지 새로고침 시에도 체크 상태 유지
- **상태**: Production Ready (2025-01-16)
...
└───────────────────┬─────────────────────────────────────┘
                    ...
                    ▼
┌─────────────────────────────────────────────────────────┐
│      Document Progress API (Backend)                    │
│      /api/v1/document-progress/                         │
│                                                          │
│      - GET  /              → 전체 진행 상황 조회         │
│      - GET  /policy/{id}   → 특정 정책 진행 상황        │
│      - POST /toggle        → 체크박스 토글               │
│      - GET  /stats         → 통계 조회                  │
└───────────────────┬─────────────────────────────────────┘
                    │
...
│      - policy_id (INT, FK → youth_policies)             │
│      - document_index (INT)                             │
...
[1. Optimistic Update (즉시 UI 업데이트)]
         │
         ▼
[2. API 호출: POST /document-progress/toggle]
         │
         ├─ Header: Authorization: Bearer <JWT>
         └─ Body: { policy_id, document_index, document_name }
         │
         ▼
[3. 백엔드: JWT 토큰 검증 → user_id 추출]
         │
         ▼
[4. 데이터베이스 조회]
         │
         ├─ 레코드 존재? → is_completed 토글 (True ↔ False)
         └─ 레코드 없음? → 새로 생성 (is_completed=True)
         │
         ▼
[5. 데이터베이스 커밋]
         │
         ▼
[6. 응답 반환: DocumentProgressResponse]
         │
         ▼
[7. 프론트엔드: API 성공 확인]
         │
...
│   └── api/v1/
│       └── document_progress.py           # API 라우터 (4개 엔드포인트)
│
├── create_document_progress_table_sql.py  # 테이블 생성 스크립트
└── DOCUMENT_PROGRESS_FEATURE.md           # 상세 문서

frontend/
└── FinKuRN/src/
    ├── services/
    │   └── documentProgressService.ts     # API 클라이언트
    └── screens/plan/
        └── PlanScreen.tsx                 # UI 구현
```
...
| 기술 | 사용 이유 |
|------|-----------|
| **PostgreSQL UUID** | users 테이블과 FK 연결 |
| **Unique Constraint** | (user_id, policy_id, document_index) 중복 방지 |
| **Optimistic Update** | 즉각적인 UI 반응성 |
| **JWT Authentication** | 사용자별 데이터 격리 |
| **ON DELETE CASCADE** | 사용자/정책 삭제 시 자동 정리 |

#### 보안

- ✅ JWT 토큰 필수 (모든 엔드포인트)
- ✅ 사용자별 데이터 격리 (타 사용자 데이터 접근 불가)
...
# 4. PlanScreen에서 체크박스 클릭 → 데이터베이스 확인
psql -U postgres -d finkurn
SELECT * FROM user_document_progress;
```

#### 상세 문서

전체 구현 내용은 다음 문서를 참고하세요:

📄 **[DOCUMENT_PROGRESS_FEATURE.md](./DOCUMENT_PROGRESS_FEATURE.md)** (600+ 줄)
